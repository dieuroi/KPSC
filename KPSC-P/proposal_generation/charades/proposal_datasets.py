...
class PROPOSAL_DATASETS(data.Dataset):
    ...
    def _parse_list(self):
        ...
        cnt = 0
        for x in tqdm(datasets_proposal_list):
            cnt += 1
            self.video_list.append(VideoRecord(x, self.datasets_root_path))
            self.full_video_list.append(VideoRecord_full(x, self.datasets_root_path))
    ...
    def __getitem__(self, index):
        ...
        full_record = self.full_video_list[index]
        segment_indices = self._get_val_indices(record)
        full_segment_indices = self._get_val_indices(full_record)
        ...
        # print(full_segment_indices)
        # print("============================================")
        assert record.vid == full_record.vid
        all_video = self.get(full_record, full_segment_indices)
        all_video_x = all_video.reshape(8, 3, 224, 224)

        begin = int(record.timestamp[0] * 8)
        end = int(record.timestamp[1] * 8)
        type_ = all_video.dtype

        zero_ = torch.zeros([end - begin,  3, 224, 224]).type(type_)

        before = all_video_x[:begin, :, :, :]
        after = all_video_x[end:, :, :, :]

        #print("zero_:", zero_.shape, before.shape, after.shape, begin, end)
        new_video = torch.cat([before, zero_, after])
        assert new_video.shape[0] == 8
        new_video = new_video.reshape(8*3, 224, 224)

        mess_dict = {}
        mess_dict['vid'] = record.vid
        mess_dict['timestamp'] = record.timestamp
        mess_dict['duration'] = record.duration

        return all_video, new_video, record.bis, mess_dict
