...
anno_path = 'ActivityNet_train_proposal_generation_seg0.06.json'
datasets_root_path = '/SSD2T/Datasets/ActivityNet_raw_frames'
down_line = 0.3
up_line = 0.4
print("down_line:", down_line, "up_line:", up_line)
batch_size = 128
...
for id, (full_image_input, sub_img_input, bis, messes) in enumerate(tqdm(anno_loader)):
    ...
    for i in range(batch_size):
        ...
        if (log > down_line) and (log < up_line):
            sub_dict = {}
...
with open('ActivityNet_train_proposal_generation_selected_propoal_downline{}_upline{}.json'.format(down_line, up_line), 'w') as file_obj:
