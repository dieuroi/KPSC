...
class CharadesBasicDatasetBuilder():
    def __init__(self, cfg, anno_train_path=None, anno_test_path=None, vid_path=None, useverbs=False):
        # make variables that will be used in the future
        self.cfg = cfg
        self.splits = ["train","test"]  # list of splits
        # make paths
        self.anno_path = {"train":anno_train_path, "test":anno_test_path}
        self.vid_path = vid_path
        # read annotations
        self.annos = self._read_annos(self.anno_path)
        # make dictionary of word-index correspondence
        self.wtoi, self.itow = self._make_word_dictionary()
    ...
    def make_dataloaders(self):
        ...
        # make dictionary of word-index correspondence
        wtoi, itow = self._make_word_dictionary()
...
class CharadesBasicDataset(Dataset):
    def __init__(self, cfg, vid_path, split, wtoi, itow, annos):
        ...
        self.sentence_max_length = self.cfg.DATASET.MAX_LENGTH#77
        assert self.sentence_max_length == 77
    ...
    def get_fixed_length_feat(self, feat, num_segment, start_pos, end_pos):
        ...
        nfeats:视频总的特征长度, num_segments:视频需要的特征长度
        """
        nfeats = feat[:,:].shape[0]
    ...
    def __getitem__(self, idx):
        ...
        instance = {
            ...
            # "query_labels": torch.LongTensor(query_label),     # [1,L_q_max]
            # "query_masks": torch.FloatTensor(query_label)>0, # [1,L_q_max]
